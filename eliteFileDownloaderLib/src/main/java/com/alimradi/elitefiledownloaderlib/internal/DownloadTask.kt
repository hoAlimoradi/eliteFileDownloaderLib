...
import com.alimradi.elitefiledownloaderlib.handler.ProgressHandler
import com.alimradi.elitefiledownloaderlib.httpclient.HttpClient
import com.alimradi.elitefiledownloaderlib.internal.stream.FileDownloadOutputStream
import com.alimradi.elitefiledownloaderlib.internal.stream.FileDownloadRandomAccessFile
...
class DownloadTask private constructor(private val request: DownloadRequest) {
    ...
    fun run(): Response {
        ...
        try {
            progressHandler = ProgressHandler(request)
            ...
            httpClient?.responseCode?.let { responseCode = it }
            ...
            if (totalBytes == 0L) {
                httpClient?.contentLength?.let { totalBytes = it }
            ...
            val path = Utils.getPath(
                request.dirPath, request.fileName
            )
